<script lang="ts">
  ...
  const privacyOptions = [
    { value: 0, label: 'Public - Everyone can see', icon: 'globe' },
    { value: 1, label: 'Almost Private - Only followers can see', icon: 'users' },
    { value: 2, label: 'Private - Only selected users can see', icon: 'lock' },
  ]

  // Add state for selected users
  let selectedUsers = new Set(); // Change from array to Set
  let privacyType = 0 // Default to public
  ...
  async function handleSubmitPost() {
    ...
    if (privacyType === 2 && selectedUsers.size === 0) {
    ...
    try {
      ...
      const postData = {
        ...
        privacy: privacyType,
        selectedUsers: privacyType === 2 ? Array.from(selectedUsers) : [], // Convert Set to array
  ...
  async function loadFollowers() {
    try {
      // Use existing endpoint with current user ID
      const response = await fetch(`http://localhost:8080/user/${$auth?.user.id}`, {
        credentials: 'include'
      });
      if (response.ok) {
...
<div class="min-h-screen bg-gradient-to-b from-gray-50 to-white">
  <div class="max-w-[90vw] mx-auto px-4 py-8 pt-10">
    ...
    {#if $auth.isAuthenticated}
      <div
        class="sticky top-20 z-40 mb-8 bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg p-6 border border-gray-100">
        ...
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div class="max-h-[calc(100vh-8rem)] overflow-y-auto">
            <form on:submit|preventDefault={handleSubmitPost} class="p-6 space-y-6">
              ...
              <!-- Image Upload Area -->
              <div
                class="relative border-2 border-dashed rounded-lg p-6 transition-all duration-200 hover:border-gray-400">
                <input
                  id="media-file"
                  type="file"
                  accept="image/*"
                ...
                <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div class="space-y-2 max-h-40 overflow-y-auto">
                    {#if $followers.followers && $followers.followers.length > 0}
                      {#each $followers.followers as user}
                        <label class="flex items-center space-x-3">
                          <input
                            type="checkbox"
                            on:change={() => handleUserSelection(user.userId)}
                      ...
                      {/each}
                    {:else}
                      <p class="text-gray-500">No onw is following you yet :(</p>
                    {/if}
                  </div>
                </div>
              {/if}
