                                ...
                                <div class="p-6">
                                    <form on:submit|preventDefault={handleSubmitPost} class="space-y-6">
                                        ...
                                        <div>
                                            ...
                                            {#if mediaFile}
                                                <div class="mt-2">
                                                    <Button on:click={clearFile} color="red" size="sm" class="w-full sm:w-auto">
                                                        Clear Image
                                                    </Button>
                                                </div>
                                            {/if}
                ...
                {#each $posts as post (post.id)}
                    <Card class="border-0 shadow-lg hover:shadow-xl transition-shadow duration-300">
                        <div class="flex justify-between items-start">
                            <div>
                                <h3 class="text-xl font-semibold mb-2 text-gray-800">{post?.title || 'Untitled'}</h3>
                        ...
                        {#if post?.media}
                            <img src={post.media} alt="Post media" class="mt-4 rounded-lg max-h-96 w-auto object-cover shadow-md cursor-pointer hover:opacity-80 transition-opacity" on:click={() => { expandedImageSrc = post.media; showExpandedImage = true; }} />
                        {/if}
                    </Card>
                {/each}
            </div>
